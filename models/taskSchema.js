...
const addSchema = Joi.object({
  title: Joi.string().max(250).required().messages({
    ...
    "string.max":
      "Tittle length should be less than or equal to 250 characters long",
  }),
  start: Joi.string().pattern(timeRegExp).required().messages({
    ...
    "string.pattern.base":
      'String field must be in format "09:00" (HH:MM 24-hour with leading 0)',
  }),
    ...
    "string.pattern.base":
      'End field should be in format "09:30" (HH:MM 24-hour with leading 0)',
  }),
  ...
  category: Joi.string()
    .valid("todo", "in-progres", "done")
    .required()
    .messages({
      "string.base": "Category field should be a string",
      "any.only":
        "Category field should be only 'todo', 'in-progres' or 'done'",
